        ...
        {actions.length === 0 ? (
              ...
              <div key={a.id} className="flex flex-wrap items-center gap-x-3 gap-y-2 px-4 py-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${a.source === "opportunity" ? "bg-sky-100 text-sky-800" : (TIER_BADGE[a.autonomyTier] ?? "bg-zinc-100 text-zinc-700")}`}>
                      {a.source === "opportunity" ? "À faire" : (TIER_LABEL[a.autonomyTier] ?? a.autonomyTier)}
                    </span>
                    <span className="truncate text-sm font-semibold text-zinc-950">{a.title}</span>
                  </div>
                  <div className="mt-0.5 line-clamp-1 text-xs text-zinc-500">{a.rationale}</div>
                </div>
                <div className="flex shrink-0 items-center gap-1.5">
                  {a.source === "opportunity" ? (
                    // Activation : c'est à TOI d'agir → "Configurer" (deep-link).
                    <>
                      {a.deepLink && (
                        <Link href={a.deepLink} className="rounded-lg bg-brand-700 px-3 py-1.5 text-xs font-semibold text-white hover:bg-brand-800">
                          Configurer →
                        </Link>
                      )}
                      <button
                        ...
                        className="px-2 text-xs text-zinc-400 hover:text-zinc-700 disabled:opacity-50"
                      >
                        Plus tard
                      </button>
                    </>
                  ) : (
                    // Anomalie : la plateforme peut agir → "Approuver".
                    <>
                      <button
                      ...
                      {a.deepLink && (
                        <Link href={a.deepLink} className="rounded-lg border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50">
                          Voir
                        </Link>
                      )}
                      <button
          ...
          {actions.length > 3 && (
            ...
            >
              {showAllActions
                ? "Réduire"
                : `Voir les ${actions.length - 3} autres action${actions.length - 3 > 1 ? "s" : ""}`}
            </button>
          )}
          </>
        )}
