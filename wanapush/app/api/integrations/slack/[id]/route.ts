// PATCH /api/integrations/slack/[id] : toggle enabled, receive*, name
// DELETE /api/integrations/slack/[id] : supprimer
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  const existing = await prisma.slackIntegration.findFirst({ where: { id, userId }, select: { id: true } });
  if (!existing) return NextResponse.json({ error: "Intégration introuvable" }, { status: 404 });

  await prisma.slackIntegration.update({ where: { id }, data: parsed.data });
