...
class Quotes_Record_Model extends Inventory_Record_Model {
	...
	/**
	 * Override getProductsForPurchaseOrder() to handle quotes without products
	 */
	public function getProductsForPurchaseOrder() {
		...
		if (!empty($recordId)) {
			...
			if ($count == 0) {
				error_log("DEBUG Quote getProductsForPurchaseOrder: No products found for quote $recordId");

				// Get quote totals from database
				$quoteData = $adb->pquery(
					"SELECT q.subtotal, q.discount_amount, q.pre_tax_total, q.total, q.taxtype, q.s_h_amount,
					        q.adjustment, q.currency_id
					 FROM vtiger_quotes q
					 WHERE q.quoteid = ?",
					array($recordId)
				);

				error_log("DEBUG Quote getProductsForPurchaseOrder: Query returned " . $adb->num_rows($quoteData) . " rows");
				...
				if ($adb->num_rows($quoteData) > 0) {
					...
					$shAmount = floatval($adb->query_result($quoteData, 0, 's_h_amount'));
					$adjustment = floatval($adb->query_result($quoteData, 0, 'adjustment'));
					$currencyId = $adb->query_result($quoteData, 0, 'currency_id');

					error_log("DEBUG Quote getProductsForPurchaseOrder: subtotal=$subtotal, total=$total, preTaxTotal=$preTaxTotal");
	...
	/**
	 * Override getProducts() to handle quotes without products
	...
	public function getProducts() {
		...
		$quoteData = $adb->pquery(
			"SELECT q.subtotal, q.discount_amount, q.pre_tax_total, q.total, q.taxtype, q.currency_id,
			        ce.label as currency_symbol
			 FROM vtiger_quotes q
			 LEFT JOIN vtiger_currency_info ci ON q.currency_id = ci.id
		...
		if ($adb->num_rows($quoteData) > 0) {
			...
			$taxtype = $adb->query_result($quoteData, 0, 'taxtype');
			$currencyId = $adb->query_result($quoteData, 0, 'currency_id');
			$currencySymbol = $adb->query_result($quoteData, 0, 'currency_symbol') ?: '€';
