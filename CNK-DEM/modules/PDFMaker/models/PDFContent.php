...
class PDFMaker_PDFContent_Model extends PDFMaker_PDFContentUtils_Model
{
    ...
    private function getInventoryProducts($module, $focus)
    {
        ...
        if (!empty($focus->id)) {
            ...
            foreach ($relatedProducts as $i => $PData) {
                ...
                $Details['P'][$i]['PRODUCTQUANTITY'] = (int)round($PData['qty' . $sequence]);
	            ...
	            $netPriceWithoutTax = $netPrice;

				$quantity = floatval($PData['qty' . $sequence]);
	            ...
	            if ('individual' === $taxType) {
		            $netPriceWithoutTax = floatval($PData['totalAfterDiscount' . $sequence]);
	            }
	...
	public static function getNumberFormat($templateId)
	{
		if (empty(self::$numberFormat[$templateId])) {
			...
			if ('email' === $templateId) {
				$result = $adb->pquery('SELECT decimals, decimal_point, thousands_separator FROM vtiger_emakertemplates_settings ');
			} else {
				$result = $adb->pquery(
