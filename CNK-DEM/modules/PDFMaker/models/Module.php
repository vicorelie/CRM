...
class PDFMaker_Module_Model extends Vtiger_Module_Model
{
    ...
    public function GetSearchSelectboxData()
    {
        ...
        for ($i = 0; $i < $num_rows; $i++) {
            ...
            $Template_Permissions_Data = $PDFMaker->returnTemplatePermissionsData($currModule, $templateid);
    ...
    public function GetListviewData($orderby = "templateid", $dir = "ASC", $request)
    {
        ...
        if ($mode != 'Blocks') {
            $status_sql = 'SELECT * FROM vtiger_pdfmaker_userstatus INNER JOIN vtiger_pdfmaker USING(templateid) WHERE userid=? AND vtiger_pdfmaker.deleted = ?';
        ...
        for ($i = 0; $i < $num_rows; $i++) {
            ...
            $listSearchParams = $this->getSearchParams($request);

            if (!empty($listSearchParams['status']) && $listSearchParams['status'] != 'status_' . $pdftemplatearray['status']) {
                continue;
            }
    ...
    public function getSearchParams($request)
    {
        ...
        foreach ($searchParams as $groupInfo) {
            ...
            foreach ($groupInfo as $fieldSearchInfo) {
                $searchData[$fieldSearchInfo[0]] = $fieldSearchInfo[2];
            }
        }
    ...
    public function GetListviewResult($orderby = "templateid", $dir = "ASC", $request = null, $all_data = true)
    {
        ...
        if ($request) {
            ...
            foreach ($listSearchParams as $listSearchKey => $listSearchValue) {
                if (in_array($listSearchKey, $Search_Types)) {
                    switch ($listSearchKey) {
                        case 'filename':
                        case 'description':
                            $search_val = "%" . $listSearchValue . "%";
                            $Search[] = "vtiger_pdfmaker." . $listSearchKey . " LIKE ?";
                            break;
                        case 'owner':
                        case 'sharingtype':
                            $search_val = $listSearchValue;
                            $Search[] = "vtiger_pdfmaker_settings." . $listSearchKey . " = ?";
                            break;
                        case 'formodule':
                            $search_val = $listSearchValue;
                        ...
                        default:
                            $search_val = $listSearchValue;
                            $Search[] = "vtiger_pdfmaker." . $listSearchKey . " = ?";
                            break;
                    }
    ...
    public function GetDetailViewData($templateid)
    {
        ...
        $Template_Permissions_Data = $PDFMaker->returnTemplatePermissionsData($pdftemplateResult["module"], $templateid);
    ...
    public function GetAvailableTemplates($currModule, $forListView = false, $recordId = false)
    {
        ...
        if (!$forListView) {
            if ($recordId) {
                $wsModule = 'Calendar' === $currModule ? vtws_getCalendarEntityType($recordId) : $currModule;
        ...
        while ($row = $adb->fetchByAssoc($result)) {
            ...
            if (!$PDFMaker->CheckTemplatePermissions($currModule, $templateId, false)) {
                continue;
            }
            ...
            if ($recordId && !$forListView) {
                ...
                if (!$PDFMaker_Display_Model->CheckDisplayConditions($row, $entityData, $currModule, $entityCache)) {
                    continue;
                }
            }
            ...
            $templatesInfo[$templateId] = [
                ...
                'is_default' => $pdfTemplateArray['is_default'],
                'order' => $pdfTemplateArray['order'],
                'disable_export_edit' => $this->getExportEdit($row['disable_export_edit']),
            ];
        }

        //when only one template is available for module, then set it as default
        if (1 === PDFMaker_Utils_Helper::count($templatesInfo)) {
            $templateId = intval(array_values($templatesInfo)[0]['template_id']);

            if (empty($templatesInfo[$templateId]['is_default'])) {
