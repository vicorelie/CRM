...
class PDFMaker_SaveAjax_Action extends Vtiger_Action_Controller
{
    ...
    public function SaveCustomLabel(Vtiger_Request $request)
    {
        ...
        if ($labelid == "") {
            ...
            $resultLabelId = $adb->pquery("SELECT label_id FROM vtiger_pdfmaker_label_keys WHERE label_key=?", array($label_key));
    ...
    public function SaveCustomLabelValues(Vtiger_Request $request)
    {
        ...
        $labelResult = $adb->pquery('SELECT label_id FROM vtiger_pdfmaker_label_keys WHERE label_key = ?', array($labelKey));
        $labelId = $adb->query_result($labelResult, 0, 'label_id');

        list($labelObjects, $languages) = $PDFMaker->GetCustomLabels();
        ...
        foreach ($languageValues as $languageId => $languageValue) {
            ...
            if ('yes' === $control) {
                ...
                $checkResult = $adb->pquery('SELECT * FROM vtiger_pdfmaker_label_vals WHERE label_id = ? AND lang_id = ?', array($labelId, $languageId));

                if ($adb->num_rows($checkResult)) {
                    $adb->pquery('UPDATE vtiger_pdfmaker_label_vals SET label_value = ? WHERE label_id = ? AND lang_id = ?', array($languageValue, $labelId, $languageId));
                } elseif (!empty($languageValue)) {
                    $adb->pquery('INSERT INTO vtiger_pdfmaker_label_vals (label_id,lang_id,label_value) VALUES  (?,?,?)', array($labelId, $languageId, $languageValue));
                }
            }
        }
