...
if (!function_exists('its4you_formatCurrencyToPDF')) {
    ...
    function its4you_formatCurrencyToPDF($value)
    {
        ...
        if (is_numeric($value)) {
            $number = number_format((float)$value, $decimals, $decimal_point, $thousands_separator);
        } else {
...
if (!function_exists('its4you_productPosition')) {
    function its4you_productPosition($param1, $comparator, $param2)
    {
        $conditions = [];
        $condition = [];
        ...
        global $PDFMaker_Product_position;
        $plusPosition = true;

        foreach ($conditions as $condition) {
            if ('Yes' === its4you_if($condition[0], $condition[1], $condition[2], 'Yes', 'No')) {
                $plusPosition = false;
            }
        }

        if ($plusPosition) {
            $PDFMaker_Product_position = (int)$PDFMaker_Product_position + 1;
        }

        return $PDFMaker_Product_position;
    }
}