        ...
        if ($num_rows4 > 0) {
            while ($row = $adb->fetchByAssoc($result4)) {
                ...
                if ($row["esentid"] == "") {
                    ...
                    $adb->pquery(
                        $sql6,
                        array($row["from_name"], $row["from_email"], $row["subject"], $row["body"], $row["type"], "", $row["pdf_template_ids"], $row["pdf_language"], $row["userid"], $row["attachments"], $row["att_documents"], "0", "0", "0", "0", "", $row["pmodule"])
                    );
                ...
                while ($row7 = $adb->fetchByAssoc($result7)) {
                    ...
                    if (!$num_rows12) {
                        ...
                        if ($row7["emailoptout"] == "1") {
                            ...
                            $adb->pquery(
                                $sql13,
                                array($row["esentid"], $row7["crmid"], "massemail|" . $to_email, "", "", "", "", "1", "unsubscribes")
                            );
                            $adb->pquery(
                            ...
                            $adb->pquery(
                                $sql8,
                                array($row["esentid"], $row7["crmid"], "massemail|" . $to_email, $to_email, "", "", "", "", "0", "", "")
                            );
                            $adb->pquery(
