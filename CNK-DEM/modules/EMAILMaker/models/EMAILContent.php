...
class EMAILMaker_EMAILContent_Model extends EMAILMaker_EMAILContentUtils_Model
{
    ...
    private static $fields_related_documents = [];
    ...
    private function getTemplateData()
    {
        ...
        self::$fields_related_documents = array_filter(explode(',', $data['fields_related_documents']));
    }
    ...
    public function getContent($convert_recipient = true, $convert_source = true, $fixImg = false)
    {
        ...
        self::$content = decode_html(self::$subject . self::$section_sep . self::$body);
    ...
    public function getFieldDisplayValue($field, $inventoryCurrency)
    {
        ...
        switch($fieldDataType) {
            ...
            case 'text':
                ...
                break;
            case 'owner':
                $fieldDisplayValue = strip_tags($field->getDisplayValue($fieldValue));
                break;
            default:
    ...
    private function replaceInventoryDetailsBlock($module, $focus, $is_related = false)
    {
        ...
        if ($is_related === false) {
            ...
            foreach ($blockTypes as $blockType) {
                ...
                if (EMAILMaker_Utils_Helper::count((array)$Products["TOTAL"][$blockType])) {
            ...
            foreach (['VAT', 'CHARGES'] as $blockType) {
                if (strpos(self::$content, '#' . $blockType . 'BLOCK_START#') !== false && strpos(self::$content, '#' . $blockType . 'BLOCK_END#') !== false) {
                    ...
                    for ($iterator = 1; $iterator < EMAILMaker_Utils_Helper::count($VExploded); $iterator++) {
                    ...
                    if (EMAILMaker_Utils_Helper::count($Products['TOTAL'][$blockType . 'BLOCK']) > 0) {
    ...
    private function getInventoryProducts($module, $focus)
    {
        if (!empty($focus->id)) {
            ...
            foreach ($relatedProducts as $i => $PData) {
                ...
                if (EMAILMaker_Utils_Helper::count($PData["subprod_qty_list" . $sequence]) > 0) {
                ...
                if ($taxtype == "individual") {
                    ...
                    $Tax_Values = array();
                    for ($tax_count = 0; $tax_count < EMAILMaker_Utils_Helper::count($tax_details); $tax_count++) {
                    ...
                    if (EMAILMaker_Utils_Helper::count($Tax_Values) > 0) {
    ...
    private function convertRelatedBlocks()
    {
        ...
        if (false !== strpos(self::$content, '#RELBLOCK')) {
            ...
            if (0 < EMAILMaker_Utils_Helper::count($relatedBlocks[1])) {
                ...
                foreach ($relatedBlocks[1] as $relBlockId) {
                    ...
                    for ($iterator = 1; $iterator < EMAILMaker_Utils_Helper::count($exploded); $iterator++) {
                    ...
                    if (EMAILMaker_Utils_Helper::count($relBlockData) > 0) {
            ...
            foreach ($html->find("rowbreak") as $pb) {
                if ($pb->outertext == self::$rowbreak) {
                    ...
                    if ($tmpPb->tag == "td") {
                        ...
                        for ($i = 0; $i < (EMAILMaker_Utils_Helper::count($partsArr) - 1); $i++) {
                            $tmpPb->innertext = $partsArr[$i];
                            $addition = '<tr>';
                            for ($j = 0; $j < EMAILMaker_Utils_Helper::count($prev_sibling_styles); $j++) {
                                $addition .= '<td ' . $prev_sibling_styles[$j] . '>&nbsp;</td>';
                            }
                            $addition .= '<td style="' . $tmpPb->getAttribute("style") . '">' . $partsArr[$i + 1] . '</td>';
                            for ($j = 0; $j < EMAILMaker_Utils_Helper::count($next_sibling_styles); $j++) {
                                $addition .= '<td ' . $next_sibling_styles[$j] . '>&nbsp;</td>';
                            }
                            $addition .= '</tr>';
                            $tmpPb->parent()->outertext = $tmpPb->parent()->outertext . $addition;
                        }
                        $toSkip = EMAILMaker_Utils_Helper::count($partsArr) - 2;
                    }
                }
            }
            self::$content = $html->save();
	...
	{
		$value = $this->getTermsAndConditions(self::$module);

		if (empty($value)) {
			$value = $this->getTermsAndConditions('Inventory');
		}

		self::$rep['$TERMS_AND_CONDITIONS$'] = nl2br($value);
	}

	public function getTermsAndConditions($module)
	{
		$result = self::$db->pquery('SELECT tandc FROM vtiger_inventory_tandc WHERE type = ?', [$module]);

		return self::$db->query_result($result, 0, 'tandc');
	}
    ...
    private function replaceLabels()
    {
        ...
        if (EMAILMaker_Utils_Helper::count(self::$relBlockModules) > 0) {
            ...
            foreach (self::$relBlockModules as $relBlockModule) {
                if ($relBlockModule != "") {
                    ...
                    $labelResult = self::$db->pquery('SELECT vtiger_field.fieldlabel FROM vtiger_field INNER JOIN vtiger_tab ON vtiger_tab.tabid = vtiger_field.tabid WHERE vtiger_tab.name = ?', [$relBlockModule]);

                    while ($labelRow = self::$db->fetchByAssoc($labelResult)) {
                        $key = $labelRow['fieldlabel'];
                        ...
                        self::$rep[$r_rbm_upper . '_' . decode_html($key) . '%'] = $value;
                        self::$rep['%R_' . strtoupper($relBlockModule) . '_' . decode_html($key) . '%'] = $value;
                    }
        ...
        if (is_array($html->find("img"))) {
            foreach ($html->find("img") as $img) {
                ...
                if (file_exists($newPath)) {
                    ...
                    $img_type = $Parts[EMAILMaker_Utils_Helper::count($Parts) - 1];
    ...
    public function getAttachments()
    {
        ...
        return array_unique(array_merge($templateAttachments, $recordsAttachments));
    }

    /**
     * @return array
     */
    public function getRelatedDocumentsRecords()
    {
        $fieldsInfo = self::$fields_related_documents;
        $recordIds = [self::$recordId];

        if (!empty($fieldsInfo)) {
            $moduleModel = Vtiger_Module_Model::getInstance(self::$module);
            $fieldModels = $moduleModel->getFieldsByType('reference');

            foreach ($fieldModels as $fieldModel) {
                foreach ($fieldsInfo as $fieldInfo) {
                    list($fieldInfoModule, $fieldInfoColumn) = explode('|', $fieldInfo);
                    $fieldColumn = $fieldModel->get('column');

                    if ($fieldColumn === $fieldInfoColumn) {
                        $fieldName = $fieldModel->get('name');
                        $recordId = (int)self::$focus->column_fields[$fieldName];

                        if (!empty($recordId) && $fieldInfoModule === getSalesEntityType($recordId)) {
                            $recordIds[] = $recordId;
                        }
                    }
                }
        ...
        return array_filter(array_unique($recordIds));
    }
    ...
    public function getAttachmentsForRecord()
    {
        ...
        $recordIds = $this->getRelatedDocumentsRecords();
        $attachments = [];

        foreach ($recordIds as $recordId) {
            $documents = ITS4YouEmails_Attachment_Model::getParentRecords($recordId);

            /** @var Documents_Record_Model $documentRecord */
            foreach ($documents as $document) {
                if (empty($document['filename']) || (!empty($folderIds) && !in_array($document['folderid'], $folderIds))) {
                    continue;
                }
