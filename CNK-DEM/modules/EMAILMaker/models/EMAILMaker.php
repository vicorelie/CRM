...
class EMAILMaker_EMAILMaker_Model extends Vtiger_Module_Model
{
        ...
        if ($owner == $user_id) {
            ...
            switch ($sharingType) {
                ...
                case 'private':
                    ...
                    if (!empty($subordinateUsers) && EMAILMaker_Utils_Helper::count($subordinateUsers) > 0) {
    ...
    public function GetAvailableTemplatesArray($currModule, $forListView = false, $recordId = false, $rawData = false, $all = false)
    {
        ...
        if (!empty($globalTemplates)) {
            $return_array[1] = !empty($return_array[1]) ? array_merge($return_array[1], $globalTemplates) : $globalTemplates;
        }
            ...
            if (count($Related_Modules) > 0) {
                ...
                while ($row = $this->db->fetchByAssoc($result)) {
                    if ($row["module"] == "PriceBooks" && $row["module"] != $select_module) {
                        $csql = "SELECT * FROM vtiger_emakertemplates_relblockcol WHERE relblockid = ? AND columnname LIKE ?";
    ...
    public function GetEMAILPDFListData($templateIds)
    {
        ...
        if (EMAILMaker_Module_Model::isPDFMakerInstalled()) {
            ...
            $sql = 'SELECT templateid, filename, module FROM vtiger_pdfmaker WHERE templateid IN (' . generateQuestionMarks($templateIds) . ')';
