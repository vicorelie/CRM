...
class PurchaseOrder_Save_Action extends Inventory_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		// CUSTOM: Calculer cf_1243 (TOTAL FORFAIT) AVANT parent::process()
		$forfaitTarif = floatval($request->get('cf_1239')) ?: 0;
		$forfaitSupplement = floatval($request->get('cf_1241')) ?: 0;
		$totalForfaitHT = $forfaitTarif + $forfaitSupplement;
		$request->set('cf_1243', $totalForfaitHT);
		...
		if (!$recordId) {
			$lastIdResult = $adb->pquery("SELECT MAX(crmid) as lastid FROM vtiger_crmentity WHERE setype = 'PurchaseOrder'", array());
		...
		$forfaitPctAcompteValue = $request->get('cf_1235');
		$forfaitPctAcompte = ($forfaitPctAcompteValue !== null && $forfaitPctAcompteValue !== '') ? floatval($forfaitPctAcompteValue) : 43;

		$forfaitPctSoldeValue = $request->get('cf_1237');
		$forfaitPctSolde = ($forfaitPctSoldeValue !== null && $forfaitPctSoldeValue !== '') ? floatval($forfaitPctSoldeValue) : 57;

		// CUSTOM: Lire cf_1247 (MONTANT ASSURANCE) et cf_1249 (TARIF POUR 1000) depuis la DB
		$assuranceResult = $adb->pquery("SELECT cf_1247, cf_1249 FROM vtiger_purchaseordercf WHERE purchaseorderid = ?", array($recordId));
		$montantAssurance = 0;
		$tarifPour1000 = 0;
		if ($adb->num_rows($assuranceResult) > 0) {
			$montantAssurance = floatval($adb->query_result($assuranceResult, 0, 'cf_1247')) ?: 0;
			$tarifPour1000 = floatval($adb->query_result($assuranceResult, 0, 'cf_1249')) ?: 0;
		}

		// CUSTOM: Calculer cf_1251 (TARIF ASSURANCE) = ((cf_1247 / 1000) - 4) * cf_1249
		$assuranceTarif = 0;
		if ($montantAssurance > 0 && $tarifPour1000 > 0) {
			$assuranceTarif = (($montantAssurance / 1000) - 4) * $tarifPour1000;
		}

		// CUSTOM: Mettre à jour cf_1251 dans la DB
		$adb->pquery("UPDATE vtiger_purchaseordercf SET cf_1251 = ? WHERE purchaseorderid = ?", array($assuranceTarif, $recordId));
		...
		if ($discountAmount == 0) {
			$discountResult = $adb->pquery("SELECT discount_amount FROM vtiger_purchaseorder WHERE purchaseorderid = ?", array($recordId));
		...
		// Mettre à jour vtiger_purchaseordercf
		$updateResult = $adb->pquery(
			"UPDATE vtiger_purchaseordercf SET cf_1243 = ?, cf_1231 = ?, cf_1233 = ? WHERE purchaseorderid = ?",
		...
		$updateResult2 = $adb->pquery(
			"UPDATE vtiger_purchaseorder SET subtotal = ?, discount_amount = ?, pre_tax_total = ?, total = ? WHERE purchaseorderid = ?",
