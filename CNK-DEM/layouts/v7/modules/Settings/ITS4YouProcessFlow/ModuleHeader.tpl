				...
				{if $MODULE neq 'Vtiger' or $smarty.request.view neq 'Index'}
					...
					{if $MODULE neq 'Vtiger'}
						...
						<span class="current-filter-name settingModuleName filter-name pull-left">
							{if $URL and $URL|strpos:$smarty.request.view eq ''}
								<a href="{$URL}">
								...
								{/if}
							{else}
								&nbsp;{if $smarty.request.extensionModule}{$smarty.request.extensionModule}{else}
								{if $SITEROAD neq ""}
									{$SITEROAD}
								{else}
									{vtranslate({$PAGETITLE}, $QUALIFIED_MODULE)}
					...
					{else}
						...
						{/if}
						<span class="current-filter-name filter-name pull-left" style='width:50%;'>
							<span class="display-inline-block">{vtranslate({$PAGETITLE}, $QUALIFIED_MODULE)}</span>
						</span>
					{/if}
				{/if}
			</div>
			<div class="col-lg-5 col-md-5 pull-right">
				<div id="appnav" class="navbar-right">
					<ul class="nav navbar-nav">
						...
						{/foreach}
						{if ITS4YouProcessFlow_Utils_Helper::count($LISTVIEW_LINKS['LISTVIEWSETTING']) gt 0}
						...
						{if ITS4YouProcessFlow_Utils_Helper::count($LISTVIEW_LINKS['LISTVIEWBASIC']) gt 0 and !in_array($MODULE, $RESTRICTED_MODULE_LIST)}
