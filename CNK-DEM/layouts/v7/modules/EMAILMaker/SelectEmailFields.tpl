        ...
        <form class="form-horizontal" id="SendEmailFormStep1" method="post" action="index.php">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    {/if}
                    <input type="hidden" name="ispdfactive" id="ispdfactive" value="{if !empty($PDFTEMPLATEIDS)}1{else}0{/if}">
                        ...
                        <div class="modal-body tabbable">
                            ...
                            {if $IS_PDFMAKER eq 'yes'}
                                ...
                                <div class="{if empty($PDFTEMPLATEIDS)}hide{/if}" id="EMAILMakerPDFTemplatesContainer">
                                    ...
                                    <div class="row">
                                        <select id="use_common_pdf_template" multiple class="form-control">
                                            {foreach from=$PDF_TEMPLATES item=PDF_TEMPLATE_DATA key=PDF_TEMPLATE_ID}
                                                {assign var=IS_PDF_TEMPLATE_SELECTED value=!empty($PDFTEMPLATEIDS) && $PDF_TEMPLATE_ID|in_array:$PDFTEMPLATEIDS}
                                                <option value="{$PDF_TEMPLATE_ID}" {if $IS_PDF_TEMPLATE_SELECTED}selected="selected"{/if}>
                                                    {$PDF_TEMPLATE_DATA.templatename}
                                                </option>
                                            {/foreach}
                                        </select>
                                    </div>
                                    ...
                                    {if 'Invoice' eq $PARENT_MODULE and EMAILMaker_Module_Model::isEInvoiceInstalled() and EMAILMaker_Module_Model::isSingleRecord($SELECTED_IDS)}
                                        <div class="row">
                                            <h5>{vtranslate('LBL_IMPORT_EINVOICE_XML','EMAILMaker')}:</h5>
                                        </div>
                                        <div class="row">
                                            {assign var=VALIDATION_RESULT value=EMAILMaker_Module_Model::validateEInvoices($SELECTED_IDS)}
                                            {if $VALIDATION_RESULT['success']}
                                                <input type="checkbox" value="1" name="is_electronic_invoice">
                                            {else}
                                ...
                                <div class="row">
                                    <span class="pull-right {if !empty($PDFTEMPLATEIDS)}hide{/if}" id="EMAILMakerPDFTemplatesBtn">
                                        <button id="addPDFMakerTemplate" class="btn btn btn-primary" onClick="return false">
                                            <i class="fa fa-plus"></i> {vtranslate('LBL_ADD_PDFMAKER_TEMPLATES','EMAILMaker')}
                                        </button>
                                    </span>
                                </div>
                            {/if}
