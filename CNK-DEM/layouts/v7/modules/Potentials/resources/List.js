...
Vtiger_List_Js("Potentials_List_Js", {}, {
	...
	registerRappelListDetection: function() {
		...
		jQuery(document).ajaxComplete(function(event, xhr, settings) {
			...
			if (settings.data && typeof settings.data === 'string' && settings.data.indexOf('action=SaveAjax') > -1) {
				...
				if (settings.data.indexOf('cf_971=') > -1 && settings.data.indexOf('cf_971=A+Rappeler') > -1) {
					// Extraire l'ID de l'enregistrement
					var recordMatch = settings.data.match(/record=(\d+)/);
					if (recordMatch) {
						var recordId = recordMatch[1];

						console.log('[RAPPEL LIST] Statut changé vers A Rappeler via AJAX, ID:', recordId);

						// Attendre que la liste soit rechargée
						setTimeout(function() {
							var $row = jQuery('tr.listViewEntries[data-id="' + recordId + '"]');
							if ($row.length > 0) {
								console.log('[RAPPEL LIST] Ligne trouvée, ouverture popup...');
								thisInstance.openRappelPopupFromList(recordId, $row);
							} else {
								console.warn('[RAPPEL LIST] Ligne non trouvée après rechargement');
							}
						}, 1000);
					}
				}
	...
	openRappelPopupFromList: function(recordId, $row) {
		...
		if (newTab) {
			console.log('[RAPPEL LIST] Onglet ouvert avec succès');
			newTab.focus();
		} else {
			console.error('[RAPPEL LIST] Impossible d\'ouvrir l\'onglet');
		}
	}
})
