...
class PDFMaker_PDFMaker_Model extends Vtiger_Module_Model
{
    ...
    public function returnTemplatePermissionsData($selected_module, $templateid)
    {
        ...
        if (!is_admin($current_user)) {
            if (!empty($selected_module) && isPermitted($selected_module, '') != "yes") {
                $result = false;
            } elseif (!empty($templateid) && $this->CheckSharing($templateid) === false) {
                $result = false;
            }
    ...
    public function getSharing($templateId)
    {
        if (!empty($this->templateSharing[$templateId])) {
            return $this->templateSharing[$templateId];
        }

        $adb = PearDatabase::getInstance();
        $result = $adb->pquery(
            'SELECT owner, sharingtype as type FROM vtiger_pdfmaker_settings WHERE templateid = ?',
            [$templateId]
        );
        ...
        if (empty($data['type'])) {
            $data = [
                'owner' => Users_Record_Model::getCurrentUserModel()->id,
                'type' => 'draft',
            ];
        }

        $this->templateSharing[$templateId] = $data;
    ...
    {
        $current_user = Users_Record_Model::getCurrentUserModel();
        $info = $this->getSharing($templateid);
        $owner = $info['owner'];
        $sharingType = $info['type'];
        $result = false;
...
//      when only one template is available for module, then set it as default
    ...
    public function CheckTemplatePermissions($selectedModule, $templateId, $die = true)
    {
        ...
        if (!is_admin($current_user)) {
            if (!empty($selectedModule) && isPermitted($selectedModule, '') != "yes") {
                $result = false;
            } elseif (!empty($templateId) && $this->CheckSharing($templateId) === false) {
                $result = false;
            }
        ...
        $sharingInfo = $this->getSharing($templateId);

        if ('draft' === $sharingInfo['type']) {
            $result = (int)$current_user->id === (int)$sharingInfo['owner'] ? $result : false;
        }
    ...
    public function clearDuplicateLinks($moduleName, $type, $label, $url)
    {
        ...
        $res = $adb->pquery('SELECT * FROM vtiger_links WHERE tabid=? AND linktype=? AND linklabel=? AND linkurl=? ORDER BY linkid DESC', array($tabId, $type, $label, $url));
        $i = 0;
    ...
    public function GetCustomLabels()
    {
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
            ...
            foreach ($labelObjects as $labelObject) {
                if (false === $labelObject->IsLangValSet($row['id'])) {
                    $labelObject->SetLangValue($row['id'], '');
                }
            }
        }

        return array($labelObjects, $languages);
    }
    ...
    public function GetProductBlockFields($select_module = "")
    {
        ...
        $Product_Fields['PRODUCT_LISTPRICEWITHOUTTAX'] = vtranslate('LBL_PRODUCT_LIST_PRICE_WITHOUT_TAX', 'PDFMaker');
    ...
    public function createPDFAndSaveFile($request, $templates, $focus,  $records, $fileName, $moduleName, $language)
    {
        ...
        if ('edit' === $request->get('mode') && 'PDFMaker' === $request->get('module')) {
        ...
        $this->db->pquery('UPDATE vtiger_notes SET filesize=?, filename=? WHERE notesid=?',
    ...
    public function saveAttachmentRelation($documentId, $recordId) {
        $this->db->pquery('INSERT INTO vtiger_seattachmentsrel (crmid, attachmentsid) VALUES (?,?)',
            [$recordId, $documentId]
        );
    }
    ...
    public function saveAttachment($documentId, $fileName, $description, $filetype, $upload_file_path, $ownerId)
    {
        ...
        $this->db->pquery('INSERT INTO vtiger_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [
                $documentId,
                $currentUser->id,
                $ownerId,
        ...
        if (PDFMaker_PDFMaker_Model::isStoredName()) {
            $this->db->pquery('INSERT INTO vtiger_attachments(attachmentsid, name, storedname, description, type, path) VALUES (?, ?, ?, ?, ?, ?)',
                [
                    $documentId,
                    $fileName,
                    $fileName,
            ...
            $this->db->pquery('INSERT INTO vtiger_attachments(attachmentsid, name, description, type, path) VALUES (?, ?, ?, ?, ?)',
                [
                    $documentId,
                    $fileName,
                    $description,
    ...
    {
        return false !== strpos($content, '#LISTVIEWBLOCK_START#') && false !== strpos($content, '#LISTVIEWBLOCK_END#');
    }
    ...
    public function getPreparedName($records, $templates, $module, $language)
    {
        ...
        foreach ($records as $record) {
            ...
            foreach ($templates as $templateId) {
                ...
                if ($PDFMakerModel->CheckTemplatePermissions($module, $templateId, false)) {
    ...
    public function GetPreparedMPDF(&$mpdf, $records, $templates, $module, $language, $preContent = array(), $set_password = true)
    {
        ...
        foreach ($records as $record) {
            ...
            foreach ($templates as $templateId) {
                ...
                if ($PDFMakerModel->CheckTemplatePermissions($module, $templateId, false) == false) {
                    $header_html = '';
                ...
                $isLandscape = ('landscape' === $Settings['orientation']);
                ...
                if (PDFMaker_Utils_Helper::count($ListViewBlocks) > 0) {
                    ...
                    $moduleModel->addAwesomeStyle($body_html);
    ...
    {
        if ('image' === $data['type'] && !empty($data['img_id'])) {
        ...
        } elseif ('text' === $data['type'] && !empty($data['text'])) {
    ...
    {
        $adb = PearDatabase::getInstance();
        $result = $adb->pquery('SELECT * FROM vtiger_attachments WHERE attachmentsid=?', array($imageId));
