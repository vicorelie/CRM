...
class PDFMaker_PDFContentUtils_Model extends Vtiger_Base_Model
{
    ...
    public function getInventoryBreaklines($id)
    {
        ...
        $result = $adb->pquery('SELECT productid, sequence, show_header, show_subtotal FROM vtiger_pdfmaker_breakline WHERE crmid=?',
            array($id)
        );
        $products = array();
        $showHeader = 0;
    ...
    public function getInventoryCurrencyInfoCustomArray($inventory_table, $inventory_id, $id)
    {
        ...
        if (!empty($id)) {
            if (!empty($inventory_table)) {
                $sql = sprintf('SELECT currency_id, %s.conversion_rate AS conv_rate, vtiger_currency_info.* FROM %s
                    INNER JOIN vtiger_currency_info ON %s.currency_id = vtiger_currency_info.id WHERE %s=?',
                    $inventory_table, $inventory_table, $inventory_table, $inventory_id
                );
            } else {
