...
class EMAILMaker_Fields_Model extends Vtiger_Base_Model
{
    ...
    public function getSelectModuleFields($module, $forfieldname = "")
    {
        ...
        if ($module == 'Calendar') {
            $b++;
            $SelectModuleFields['Calendar'][strtolower($forfieldname . '_CRMID')] = 'Record ID';
        ...
        foreach ($Blocks as $block_label => $block_fields) {
            ...
            if (EMAILMaker_Utils_Helper::count($block_fields) > 0) {
    ...
    public static function getSignatureOptions()
    {
        ...
        $signatureModuleName = 'ITS4YouSignature';

        if (!getTabid($signatureModuleName) || !vtlib_isModuleActive($signatureModuleName) || !method_exists('ITS4YouSignature_Record_Model', 'getEmailMessageVariables')) {
            return [];
        }

        $variables = [
            '' => vtranslate('LBL_PLS_SELECT', $moduleName),
        ];
        /** @var ITS4YouSignature_Record_Model $signatureRecordModel */
        $signatureRecordModel = Vtiger_Record_Model::getCleanInstance($signatureModuleName);

        if ($signatureRecordModel) {
            $signatureVariables = array_keys(
                $signatureRecordModel->getEmailMessageVariables()
            );

            foreach ($signatureVariables as $signatureVariable) {
                $variables[$signatureVariable] = vtranslate($signatureVariable, $moduleName);
            }
        }
