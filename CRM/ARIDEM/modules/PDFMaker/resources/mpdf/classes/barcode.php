...
class PDFBarcode
{
	...
	protected function barcode_code93($code)
	{
		...
		$clen = strlen($code);
		for ($i = 0; $i < $clen; ++$i) {
			if (ord(substr($code, $i, 1)) > 127) {
				return false;
			}
			$code_ext .= $encode[substr($code, $i, 1)];
		}
		// checksum
	...
	protected function checksum_code93($code)
	{
		...
		$check = 0;
		for ($i = ($len - 1); $i >= 0; --$i) {
			$k = array_keys($chars, substr($code, $i, 1));
			$check += ($k[0] * $p);
			++$p;
			if ($p > 20) {
				$p = 1;
	...
	protected function barcode_c128($code, $type = 'B', $ean = false)
	{
		...
		switch (strtoupper($type)) {
			...
			case 'C': {
					...
					$hclen = (strlen($code) / 2);
					for ($i = 0; $i < $hclen; ++$i) {
						$new_code .= chr(intval(substr($code, (2 * $i), 1) . substr($code, (2 * $i + 1), 1)));
					}
					$code = $new_code;
					break;
				}
	...
	protected function barcode_eanupc($code, $len = 13)
	{
		...
		if ($upce) {
			...
			$p = $upce_parities[substr($code, 1, 1)][$r];
			for ($i = 0; $i < 6; ++$i) {
	...
	protected function barcode_eanext($code, $len = 5)
	{
		...
		if ($len == 2) {
			...
			$r = (3 * (substr($code, 0, 1) + substr($code, 2, 1) + substr($code, 4, 1))) + (9 * (substr($code, 1, 1) + substr($code, 3, 1)));
			$r %= 10;
		} else {
			return false;
		}
	...
	protected function barcode_imb($code)
	{
		...
		$binary_code = bcadd($binary_code, substr($tracking_number, 0, 1));
		$binary_code = bcmul($binary_code, 5);
		$binary_code = bcadd($binary_code, substr($tracking_number, 1, 1));
