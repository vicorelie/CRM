...
Emails_MassEdit_Js('ITS4YouEmails_MassEdit_Js', {
    ...
    replyEmail(recordId, recordModule) {
        let self = new ITS4YouEmails_MassEdit_Js(),
            params = {
    ...
    registerAutoCompleteFields: function (container, emailType) {
        ...
        container.find('#email' + type + 'Field').select2({
            ...
            tokenSeparators: [','],
            ajax: {
                'url': 'index.php?module=ITS4YouEmails&action=IndexAjax&mode=searchEmails',
                'dataType': 'json',
                'data': function (term, page) {
                    return {
                        'searchValue': term.trim(),
                    };
                },
    ...
    registerEventDocumentsListClick: function () {
        ...
        if (this.documentClickEventRegistered) {
            return true;
        }

        this.documentClickEventRegistered = true;
    ...
    registerEventEmailTemplateListClick: function () {
        ...
        if (self.templateClickEventRegistered) {
            return true;
        }

        self.templateClickEventRegistered = true;
    ...
    showPopupModal: function (data) {
        if (data) {
            ...
            let element = jQuery('<div class="modal popupModal"></div>');
            element.append(data);
    ...
    registerEventForRemoveCustomAttachments: function () {
        ...
        composeEmailForm.on('click', '.removeAttachment', function () {
            let attachmentsContainer = composeEmailForm.find('[name="attachments"]'),
                ...
                element = jQuery(this),
                imageContainer = element.closest('div.MultiFile-label'),
