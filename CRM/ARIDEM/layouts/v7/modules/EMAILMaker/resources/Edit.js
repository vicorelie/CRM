        ...
        if (response['success']) {
            ...
            if ('modulefields' === selectName) {
                const relatedModuleSourceElement = jQuery('#relatedmodulesorce'),
                    relatedDocumentsFields = $('#folders_related_documents');

                relatedModuleSourceElement.select2('destroy');
                relatedDocumentsFields.select2('destroy');

                relatedModuleSourceElement.empty();
                relatedDocumentsFields.empty();

                jQuery.each(response['related_modules'], function (i, item) {
                    let option = '<option value="' + item[3] + '|' + item[0] + '" data-module="' + item[3] + '">' + item[2] + ' (' + item[1] + ')' + '</option>';

                    relatedModuleSourceElement.append(option);
                    relatedDocumentsFields.append(option);
                });

                vtUtils.showSelect2ElementView(relatedModuleSourceElement);
                relatedModuleSourceElement.trigger('change');

                vtUtils.showSelect2ElementView(relatedDocumentsFields);
                relatedDocumentsFields.trigger('change');
