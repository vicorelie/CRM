...
if (!function_exists('datetimefmt')) {
    function datetimefmt($date, $outFormat = 'd.m.Y H:i')
    {
        if(empty($date)) {
            return '';
        }

        $dateInfo = explode(' ', $date, 2);

        if(empty($dateInfo[0]) || empty($dateInfo[1])) {
            return '';
        }

        return date($outFormat, strtotime(DateTimeField::convertToDBFormat($dateInfo[0]) . ' ' . $dateInfo[1]));
    }
}
