...
// --------------------------------------------------------------------------------
// 3) Préparation des filtres (même design que studyList)
// --------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------
// 5) Construction de la requête principale pour lister les QCM
// --------------------------------------------------------------------------------
try {
    // 5.a) Requête de comptage
    $countQuery = "
        SELECT COUNT(DISTINCT dq.id)
        FROM documentQuestions dq
        INNER JOIN subjectDocuments SD ON dq.subject_document_id = SD.id
    ...
    if ($filterDate !== '') {
        ...
        if (count($dates) === 2) {
            $countQuery .= " AND DATE(dq.created_time) BETWEEN :start_date AND :end_date";
            $params[':start_date'] = $dates[0];
            $params[':end_date']   = $dates[1];
        } elseif (count($dates) === 1) {
            $countQuery .= " AND DATE(dq.created_time) = :exact_date";
    ...
    $totalQCM = $stmtTotal->fetchColumn();
    $totalPages = ceil($totalQCM / $limit);

    // 5.b) Requête principale
    // On veut un regroupement par subject_document_id
    $query = "
        SELECT 
            ...
            MIN(dq.created_time)   AS first_qcm_date, 
            COUNT(dq.id)           AS nb_qcm,
    ...
    if ($filterDate !== '') {
        if (count($dates) === 2) {
            $query .= " AND DATE(dq.created_time) BETWEEN :start_date AND :end_date";
        } elseif (count($dates) === 1) {
            $query .= " AND DATE(dq.created_time) = :exact_date";
        }
    }
    if ($generalSearch !== '') {
        $query .= " 
...
// --------------------------------------------------------------------------------
// 6) Récupération des soumissions (qcmSubmit) groupées par subject_document_id
// --------------------------------------------------------------------------------
$submissionsByDoc = [];
if (!empty($qcmList)) {
    // Extraire les IDs
    $docIds = array_column($qcmList, 'subject_document_id');
    $docIds = array_filter($docIds); // enlever les potentiels nuls
    if (!empty($docIds)) {
        ...
        $submissionsQuery = "
            SELECT 
                qs.id, 
                qs.subject_document_id,
                qs.created_time,
                qs.submitNote,
                qs.submitAnswer
            FROM qcmSubmit qs
            WHERE qs.uuid = ? 
            AND qs.subject_document_id IN ($placeholders)
            ORDER BY qs.created_time DESC
        ";
    

        $stmtSubmissions = $pdo->prepare($submissionsQuery);
        $params = array_merge([$userUuid], $docIds);
        $stmtSubmissions->execute($params);

        $allSubs = $stmtSubmissions->fetchAll(PDO::FETCH_ASSOC);
        foreach ($allSubs as $rowSub) {
            $sdId = $rowSub['subject_document_id'];
            $submissionsByDoc[$sdId][] = $rowSub;
        }
    }
}
?>
...
<head>
    ...
    <style>
        /* Reprenez ici les styles principaux de studyList pour assurer le même design */
        .hidden {
...
<div class="container py-5">
    <!-- Titre de la page -->
    <h2 class="mb-4 text-center" style="font-size:36px;" ><?= htmlspecialchars($lang_data['qcm_list_title'] ?? 'Liste de QCM') ?></h2>
    <!-- BOUTON FILTRER -->
    ...
    <div id="filtersContainer" class="card mb-4 p-3 hidden">
        <div class="card-body">
            <form method="GET" action="quizList.php" class="row g-3 align-items-center filter-form">
                <!-- Matière, Topic, Study -->
    ...
    <?php if (empty($qcmList)): ?>
        ...
        <?= htmlspecialchars($lang_data['no-quiz-finded'] ?? 'No quizzes finded') ?>
        </div>
    <?php else: ?>
        <div class="row g-4">
            <?php 
            foreach ($qcmList as $row):
                ...
                $firstQCMDate   = $row['first_qcm_date']      ?? null;
                $hasSubmissions = !empty($submissionsByDoc[$sdId]); // Voir s'il y a des soumissions
                ...
                $hasExtractedContent = !empty($extract);
            ?>
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <?php if ($firstQCMDate): ?>
                            <p class="text-muted mb-1">
                                ...
                                <?= htmlspecialchars(date('d/m/Y H:i', strtotime($firstQCMDate))) ?>
                            </p>
                        <?php endif; ?>
                        ...
                        <div class="mt-auto">
                            ...
                            <div class="row mb-2">
                                <div class="col-12">
                                    <a href="questionForm.php?subject_document_id=<?= urlencode($sdId) ?>"
                            ...
                            <!-- Bouton "Voir les soumissions" s'il y en a -->
                            <?php if ($hasSubmissions): ?>
                                <button 
                                    class="btn btn-secondary btn-sm w-100"
                                    data-bs-toggle="modal"
                                    data-bs-target="#submissionsModal-<?= $sdId ?>"
                                >
                                <i class="fas fa-chevron-down"></i> 
                                <?= $lang_data['view_submissions'] ?? 'Voir les soumissions' ?>
                                </button>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <!-- Modal Soumissions (si $hasSubmissions) -->
            <?php if ($hasSubmissions): ?>
            <div 
              class="modal fade" 
              id="submissionsModal-<?= $sdId ?>" 
              tabindex="-1" 
              aria-labelledby="submissionsModalLabel-<?= $sdId ?>"
              aria-hidden="true"
            >
              <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="submissionsModalLabel-<?= $sdId ?>">
                    <?= htmlspecialchars($lang_data['quiz_submit_title'] ?? 'Resultats de quiz') ?> - <?= htmlspecialchars($topic) ?>
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                  </div>
                  <div class="modal-body">
                    <div class="list-group">
                      <?php foreach ($submissionsByDoc[$sdId] as $sub): ?>
                        <a 
                          href="qcmResult.php?subject_document_id=<?= urlencode($sdId) ?>&submit_id=<?= urlencode($sub['id']) ?>"
                          class="list-group-item list-group-item-action d-flex justify-content-between align-items-center"
                        >
                          <div>
                            <strong><?= htmlspecialchars($lang_data['quiz_results'] ?? 'Resultats du quiz') ?></strong><br>
                          ...
                          <div class="text-end">
                            <?php 
                            // Nombre de bonnes réponses à partir du champ submitAnswer
                            $correctAnswers = isset($sub['submitNote']) ? (int)$sub['submitNote'] : 0;
                        
                            // Nombre total de questions
                            $totalAnswers = 0;
                            if (!empty($sub['submitAnswer'])) {
                                $answers = explode(',', $sub['submitAnswer']);
                                $totalAnswers = count($answers);
                            }
                        
                            // Conversion en note sur 100
                            $scorePercent = $totalAnswers > 0
                                ? round($correctAnswers / $totalAnswers * 100)
                                : 0;
                        ?>
                        <span class="badge bg-success">
                            <?= htmlspecialchars($lang_data['qcm_result_score'] ?? 'Score') ?>: 
                            <?= $scorePercent ?>/100
                        </span>
