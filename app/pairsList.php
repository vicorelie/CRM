...
// --------------------------------------------------------------------------------
// 3) Préparation des filtres (même design que summaryList/quizList)
// --------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------
// 5) Construction de la requête principale pour lister les pairs
// --------------------------------------------------------------------------------
try {
    // 5.a) Requête de comptage
    $countQuery = "
        SELECT COUNT(DISTINCT dp.id)
        FROM documentPairs dp
        INNER JOIN subjectDocuments SD ON dp.subject_document_id = SD.id
    ...
    if ($filterDate !== '') {
        ...
        if (count($dates) === 2) {
            $countQuery .= " AND DATE(dp.created_time) BETWEEN :start_date AND :end_date";
            $params[':start_date'] = $dates[0];
            $params[':end_date']   = $dates[1];
        } elseif (count($dates) === 1) {
            $countQuery .= " AND DATE(dp.created_time) = :exact_date";
    ...
    $totalPairs = $stmtTotal->fetchColumn();
    $totalPages = ceil($totalPairs / $limit);

    // 5.b) Requête principale
    // On regroupe par subject_document_id (même logique que summaryList).
    $query = "
        SELECT 
            ...
            MIN(dp.created_time) AS first_pairs_date,
            COUNT(dp.id)         AS nb_pairs,
    ...
    if ($filterDate !== '') {
        if (count($dates) === 2) {
            $query .= " AND DATE(dp.created_time) BETWEEN :start_date AND :end_date";
        } elseif (count($dates) === 1) {
            $query .= " AND DATE(dp.created_time) = :exact_date";
        }
    }
    if ($generalSearch !== '') {
        $query .= " 
    ...
    $query .= "
        GROUP BY SD.id
        ORDER BY first_pairs_date DESC
    ...
    $pairsList = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    <meta charset="UTF-8">
    <title>Liste de Pairs</title>
    <!-- Flatpickr, FontAwesome, etc. -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        /* Reprenez ici les styles principaux de summaryList/quizList pour assurer le même design */
        .hidden {
...
<div class="container py-5">
    ...
    <h2 class="mb-4 text-center" style="font-size:36px;">
        <?= htmlspecialchars($lang_data['pairs_list_title'] ?? 'Liste de Pairs') ?>
    </h2>
    <!-- BOUTON FILTRER -->
    ...
    <div id="filtersContainer" class="card mb-4 p-3 hidden">
        <div class="card-body">
            <form method="GET" action="pairsList.php" class="row g-3 align-items-center filter-form">
                <!-- Matière, Topic, Study -->
    ...
    <!-- LISTE DES PAIRS -->
    <?php if (empty($pairsList)): ?>
        <div class="alert alert-info text-center">
        <?= htmlspecialchars($lang_data['no-pairs-finded'] ?? 'No pairs finded') ?>
        </div>
    <?php else: ?>
        <div class="row g-4">
            <?php 
            foreach ($pairsList as $row):
                ...
                $firstPairsDate  = $row['first_pairs_date']    ?? null;
            ...
            <div class="col-lg-4 col-md-6">
                <div class="card shadow-sm h-100">
                    <div class="card-body d-flex flex-column">
                        ...
                        <!-- Date + matière -->
                        <?php if ($firstPairsDate): ?>
                            <p class="text-muted mb-1">
                                <i class="far fa-calendar-alt"></i>
                                <?= htmlspecialchars(date('d/m/Y H:i', strtotime($firstPairsDate))) ?>
                            </p>
                        <?php endif; ?>
                        ...
                        <!-- Bouton d'action (voir pair) -->
                        <div class="mt-auto">
                            <div class="row mb-2">
                                <div class="col-12">
                                    <a href="viewPair.php?subject_document_id=<?= urlencode($sdId) ?>"
                                       class="btn btn-primary btn-sm w-100 mb-2">
                                       <i class="fas fa-arrows-alt-h"></i>
                                       <?= htmlspecialchars($lang_data['view_pairs'] ?? 'Voir Pair') ?>
                                    </a>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            window.location.href = 'pairsList.php';
        });
    }
