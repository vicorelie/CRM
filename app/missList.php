...
// --------------------------------------------------------------------------------
// 3) Préparation des filtres (même design que flashList/pairsList/summaryList/quizList)
// --------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------
// 5) Construction de la requête principale pour lister les miss
// --------------------------------------------------------------------------------
try {
    // 5.a) Requête de comptage
    $countQuery = "
        SELECT COUNT(DISTINCT dm.id)
        FROM documentMiss dm
        INNER JOIN subjectDocuments SD ON dm.subject_document_id = SD.id
    ...
    if ($filterDate !== '') {
        ...
        if (count($dates) === 2) {
            $countQuery .= " AND DATE(dm.created_time) BETWEEN :start_date AND :end_date";
            $params[':start_date'] = $dates[0];
            $params[':end_date']   = $dates[1];
        } elseif (count($dates) === 1) {
            $countQuery .= " AND DATE(dm.created_time) = :exact_date";
    ...
    $totalMiss = $stmtTotal->fetchColumn();
    $totalPages = ceil($totalMiss / $limit);

    // 5.b) Requête principale
    // On regroupe par subject_document_id (même logique que flashList/pairsList).
    $query = "
        SELECT 
            ...
            MIN(dm.created_time) AS first_miss_date,
            COUNT(dm.id)         AS nb_miss,
    ...
    if ($filterDate !== '') {
        if (count($dates) === 2) {
            $query .= " AND DATE(dm.created_time) BETWEEN :start_date AND :end_date";
        } elseif (count($dates) === 1) {
            $query .= " AND DATE(dm.created_time) = :exact_date";
        }
    }
    if ($generalSearch !== '') {
        $query .= " 
    ...
    $missList = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    <meta charset="UTF-8">
    <title>Liste des Miss</title>
    <!-- Flatpickr, FontAwesome, etc. -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        /* Reprenez ici les styles principaux de flashList/pairsList/summaryList/quizList pour assurer le même design */
        .hidden {
...
<div class="container py-5">
    ...
    <h2 class="mb-4 text-center" style="font-size:36px;">
        <?= htmlspecialchars($lang_data['miss_list_title'] ?? 'Liste de Miss') ?>
    </h2>
    <!-- BOUTON FILTRER -->
    ...
    <div id="filtersContainer" class="card mb-4 p-3 hidden">
        <div class="card-body">
            <form method="GET" action="missList.php" class="row g-3 align-items-center filter-form">
                <!-- Matière, Topic, Study -->
    ...
    <?php if (empty($missList)): ?>
        ...
        <?= htmlspecialchars($lang_data['no-miss-finded'] ?? 'No miss finded') ?>
        </div>
    <?php else: ?>
        <div class="row g-4">
            <?php 
            foreach ($missList as $row):
                ...
                $firstMissDate = $row['first_miss_date']     ?? null;
            ...
            <div class="col-lg-4 col-md-6">
                <div class="card shadow-sm h-100">
                    <div class="card-body d-flex flex-column">
                        ...
                        <!-- Date + matière -->
                        <?php if ($firstMissDate): ?>
                            <p class="text-muted mb-1">
                                <i class="far fa-calendar-alt"></i>
                                <?= htmlspecialchars(date('d/m/Y H:i', strtotime($firstMissDate))) ?>
                            </p>
                        <?php endif; ?>
                        ...
                        <!-- Bouton d'action (voir miss) -->
                        <div class="mt-auto">
                            <div class="row mb-2">
                                <div class="col-12">
                                    <a href="viewMiss.php?subject_document_id=<?= urlencode($sdId) ?>"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            window.location.href = 'missList.php';
        });
    }
