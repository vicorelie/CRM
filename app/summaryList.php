...
// --------------------------------------------------------------------------------
// 3) Préparation des filtres (même design que studyList / quizList)
// --------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------
// 5) Construction de la requête principale pour lister les résumés
// --------------------------------------------------------------------------------
try {
    // 5.a) Requête de comptage
    $countQuery = "
        SELECT COUNT(DISTINCT dr.id)
        FROM documentResumes dr
        INNER JOIN subjectDocuments SD ON dr.subject_document_id = SD.id
    ...
    if ($filterDate !== '') {
        ...
        if (count($dates) === 2) {
            $countQuery .= " AND DATE(dr.created_time) BETWEEN :start_date AND :end_date";
            $params[':start_date'] = $dates[0];
            $params[':end_date']   = $dates[1];
        } elseif (count($dates) === 1) {
            $countQuery .= " AND DATE(dr.created_time) = :exact_date";
    ...
    $totalSummaries = $stmtTotal->fetchColumn();
    $totalPages = ceil($totalSummaries / $limit);

    // 5.b) Requête principale
    // On regroupe par subject_document_id (même logique que quizList).
    $query = "
        SELECT 
            ...
            MIN(dr.created_time)      AS first_summary_date,
            COUNT(dr.id)             AS nb_summaries,
    ...
    if ($filterDate !== '') {
        if (count($dates) === 2) {
            $query .= " AND DATE(dr.created_time) BETWEEN :start_date AND :end_date";
        } elseif (count($dates) === 1) {
            $query .= " AND DATE(dr.created_time) = :exact_date";
        }
    }
    if ($generalSearch !== '') {
        $query .= " 
    ...
    $query .= "
        GROUP BY SD.id
        ORDER BY first_summary_date DESC
    ...
    $summaryList = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    <meta charset="UTF-8">
    <title>Liste de Résumés</title>
    <!-- Flatpickr, FontAwesome, etc. -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        /* Reprenez ici les styles principaux de studyList/quizList pour assurer le même design */
        .hidden {
...
<div class="container py-5">
    ...
    <h2 class="mb-4 text-center" style="font-size:36px;" >
        <?= htmlspecialchars($lang_data['summary_list_title'] ?? 'Liste de Résumés') ?>
    </h2>
    <!-- BOUTON FILTRER -->
    ...
    <div id="filtersContainer" class="card mb-4 p-3 hidden">
        <div class="card-body">
            <form method="GET" action="summaryList.php" class="row g-3 align-items-center filter-form">
                <!-- Matière, Topic, Study -->
    ...
    <?php if (empty($summaryList)): ?>
        <div class="alert alert-info text-center">
            <?= htmlspecialchars($lang_data['no-summary-finded'] ?? 'No summary finded') ?>
        </div>
    <?php else: ?>
        <div class="row g-4">
            <?php 
            foreach ($summaryList as $row):
                ...
                $firstSummaryDate = $row['first_summary_date'] ?? null;
            ...
            <div class="col-lg-4 col-md-6">
                <div class="card shadow-sm h-100">
                    <div class="card-body d-flex flex-column">
                        ...
                        <!-- Date + matière -->
                        <?php if ($firstSummaryDate): ?>
                            <p class="text-muted mb-1">
                                <i class="far fa-calendar-alt"></i>
                                <?= htmlspecialchars(date('d/m/Y H:i', strtotime($firstSummaryDate))) ?>
                            </p>
                        <?php endif; ?>
                        ...
                        <!-- Bouton d'action (voir résumé) -->
                        <div class="mt-auto">
                            <div class="row mb-2">
                                <div class="col-12">
                                    <a href="viewSummary.php?subject_document_id=<?= urlencode($sdId) ?>"
                            ...
                            <!-- Pas de second bouton ici, contrairement à quizList -->
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            window.location.href = 'summaryList.php';
        });
    }
