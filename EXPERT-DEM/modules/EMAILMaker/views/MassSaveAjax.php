...
class EMAILMaker_MassSaveAjax_View extends Vtiger_Footer_View
{
    ...
    public function massSave(Vtiger_Request $request)
    {
        ...
        if (EMAILMaker_Utils_Helper::count($_FILES) > 0) {
        ...
        if (EMAILMaker_Utils_Helper::count($documentIds)) {
    ...
    public function getRecordsListFromRequest(Vtiger_Request $request)
    {
        ...
        if (!empty($selectedIds) && $selectedIds != 'all') {
            if (!empty($selectedIds) && EMAILMaker_Utils_Helper::count($selectedIds) > 0) {
    ...
    public function saveEmails($Send_Emails, $SendMail, $language, $pdf_template_ids, $pdflanguage, $ids_for_pdf)
    {
        ...
        if (EMAILMaker_Utils_Helper::count($Send_Emails) > 0) {
            foreach ($Send_Emails as $SED) {
                ...
                foreach ((array)$SendMail["to"] as $sid => $PidsEmails) {
                    ...
                    foreach ($ctypes as $c_type) {
                        if (EMAILMaker_Utils_Helper::count($SendMail[$c_type][$sid]) > 0) {
                    ...
                    if (EMAILMaker_Utils_Helper::count($CEmails[$sid]["cc"]) > 0) {
                        $cc = implode(", ", $CEmails[$sid]["cc"]);
                    }
                    if (EMAILMaker_Utils_Helper::count($Email_IDs_Array[$sid]["cc"]) > 0) {
                        $cc_ids = implode(", ", $Email_IDs_Array[$sid]["cc"]);
                    }
                    if (EMAILMaker_Utils_Helper::count($CEmails[$sid]["bcc"]) > 0) {
                        $bcc = implode(", ", $CEmails[$sid]["bcc"]);
                    }
                    if (EMAILMaker_Utils_Helper::count($Email_IDs_Array[$sid]["bcc"]) > 0) {
    ...
    public function checkUploadSize($documentIds = false)
    {
        ...
        if (!empty ($documentIds)) {
            $count = EMAILMaker_Utils_Helper::count($documentIds);
            for ($i = 0; $i < $count; $i++) {
