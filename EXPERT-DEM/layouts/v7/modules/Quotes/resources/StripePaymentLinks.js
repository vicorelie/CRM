    ...
    if (data.payments && data.payments.length > 0) {
        ...
        data.payments.forEach(function(payment) {
            ...
            if (payment.invoice_id) {
                invoiceBtn = `<a href="index.php?module=PDFMaker&action=CreatePDFFromTemplate&mode=CreatePDF&source_module=Invoice&formodule=Invoice&record=${payment.invoice_id}&pdftemplateid=55" target="_blank" class="btn btn-xs btn-success" style="${smallBtnStyle}" title="Voir la facture PDF"><i class="fa fa-file-pdf-o"></i></a>`;
            }
...
function generatePaymentEmailHTML(data, paymentLink, amount, description) {
    ...
    var logoUrl = data.company_logo ? ('https://crm.expertdem.com/' + data.company_logo) : '';
    var companyName = data.company_name || 'EXPERT-DEM';
