...
class Settings_ITS4YouInstaller_Requirements_Model extends Vtiger_Base_Model
{
    ...
    public function getValueCharset()
    {
     ...
     * @return string
     */
    public function getValueSiteUrl()
    {
        global $site_URL;
     ...
     * @return string
     */
    public function getValueRootDirectory()
    {
    ...
    public function isWritableFolder($dir)
    {
        ...
        if (isset($_REQUEST['scan']) && 'SubFolders' === $_REQUEST['scan']) {
            $files = scandir($dir);
    ...
    public function retrieveDBSettings()
    {
        foreach ($this->getDBSettings() as $key => $data) {
            $data = $this->retrieveDefaultValues($data);
            $data['name'] = $key;
    ...
    public function retrieveDefaultValues($data)
    {
        $keys = ['warning', 'error', 'info', 'recommended_description',];
    ...
    {
        if (isset($data['error']) && 'yes' === $data['error']) {
            $this->buttonType = 'danger';
        } elseif ('danger' !== $this->buttonType && isset($data['warning']) && 'yes' === $data['warning']) {
