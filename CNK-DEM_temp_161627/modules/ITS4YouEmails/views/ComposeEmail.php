...
class ITS4YouEmails_ComposeEmail_View extends Vtiger_ComposeEmail_View
{
	...
	public function retrieveEInvoice(Vtiger_Request $request)
	{
		if ($request->isEmpty('is_electronic_invoice') || !vtlib_isModuleActive('ITS4YouEInvoice')) {
			return;
		}
		...
		foreach ($selectedIds as $selectedId) {
			if (!empty($selectedId) && 'Invoice' === getSalesEntityType($selectedId)) {
				$record = ITS4YouEInvoice_Record_Model::getInstanceByInvoiceId($selectedId);
				$record->saveDocument();
				$documentIds[] = $record->getDocumentId();
			}
		}

		$request->set('document_ids', $documentIds);
	}
	...
	public function retrieveAttachments(Vtiger_Request $request)
    {
        ...
        if ($this->EMAILContentModel) {
            $documentIds = array_merge($documentIds, $this->EMAILContentModel->getAttachments());
        }
