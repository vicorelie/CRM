...
class PDFMaker_SaveRelatedBlock_Action extends Vtiger_Action_Controller
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if ($this->relblockid != "") {
            $adb->pquery("UPDATE vtiger_pdfmaker_relblocks SET name=?, block=? WHERE relblockid=?", array($name, $block, $this->relblockid));
        } else {
            $this->relblockid = $adb->getUniqueID('vtiger_pdfmaker_relblocks');
            $adb->pquery("INSERT INTO vtiger_pdfmaker_relblocks (relblockid, name, module, secmodule, block) VALUES (?,?,?,?,?)", array($this->relblockid, $name, $module, $secmodule, $block));
